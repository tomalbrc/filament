...
public class Armor implements ItemBehaviour<Armor.Config> {
    ...
    public void modifyPolymerItemStack(Map<String, ResourceLocation> modelMap, ItemStack original, ItemStack polymer, TooltipFlag tooltipFlag, HolderLookup.Provider lookup, ServerPlayer player) {
        if (this.trimHolder != null)
            polymer.set(DataComponents.TRIM, new ArmorTrim(lookup.lookup(Registries.TRIM_MATERIAL).orElseThrow().get(TrimMaterials.QUARTZ).orElseThrow(), this.trimHolder.trimPattern, false));
    }
